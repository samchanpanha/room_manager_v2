// Builds a self-contained single-page administrator documentation site from
// the admin-guide .md files, in the app's three switchable languages:
//   en — English (docs/admin-guide/*.md — source of truth)
//   km — Khmer   (docs/admin-guide/km/*.md)
//   zh — Chinese (docs/admin-guide/zh/*.md)
// Run: node docs/admin-guide/site/build.mjs   ->   docs/admin-guide/site/index.html
// Also published into the Next.js public/ folder (in-app at /admin-guide).
//
...
const PARTS = [
  { file: "01-what-rentmanager-is.md", g: 0 },
  { file: "02-architecture.md", g: 0 },
  { file: "03-first-login.md", g: 0 },
  { file: "04-rbdc.md", g: 1 },
  { file: "05-user-management.md", g: 1 },
  { file: "06-organisation-setup.md", g: 1 },
  { file: "07-settings.md", g: 2 },
  { file: "08-module-operations.md", g: 2 },
  { file: "09-scheduled-jobs.md", g: 3 },
  { file: "10-backup-restore.md", g: 3 },
  { file: "11-security-hardening.md", g: 3 },
  { file: "12-monitoring.md", g: 3 },
  { file: "13-updating.md", g: 4 },
  { file: "14-troubleshooting.md", g: 4 },
  { file: "15-go-live.md", g: 4 }
];
...
for (const loc of SITE_LOCALES) {
  partsByLocale[loc] = PARTS.map((p) => {
    ...
    if (fs.existsSync(pAbs)) {
      ...
      console.warn(`[admin-guide] missing ${rel} — using the English text for this part`);
    ...
    if (h2s.length !== enList.length) {
      console.warn(`[admin-guide] ${loc}/${p.file}: ${h2s.length} "## " sections vs ${enList.length} in English — anchors may drift`);
    }
...
// ── Structural checks (walkthroughs & diagrams mirror English 1:1) ──
{
  for (const loc of SITE_LOCALES) {
    ...
    if (WALKS[loc].length !== WALKS.en.length) {
      console.warn(`[admin-guide] ${loc}: ${WALKS[loc].length} walkthroughs vs ${WALKS.en.length} in English`);
    }
    WALKS.en.forEach((w, i) => {
      const l = WALKS[loc][i];
      if (!l || l.id !== w.id) console.warn(`[admin-guide] ${loc}: walkthrough #${i + 1} id mismatch (${w.id} vs ${l && l.id})`);
      else if (l.steps.length !== w.steps.length) console.warn(`[admin-guide] ${loc}: walkthrough "${w.id}" has ${l.steps.length} steps vs ${w.steps.length} in English`);
    });
    for (const [partSlug, table] of Object.entries(DIAGRAMS.en)) {
      const lt = DIAGRAMS[loc][partSlug] || {};
      for (const key of Object.keys(table)) {
        if (!lt[key]) console.warn(`[admin-guide] ${loc}: missing diagram ${partSlug} → ${key}`);
      }
    }
  }
}
...
function renderHome() {
  ...
  main.innerHTML =
   ...
   '<a class="btn ghost" href="#/part/03-first-login">' + u.home.btnQuick + '</a></div>' +
   '<div class="hero-img"><img src="' + IMG.hero + '" alt="RentManager illustration"/></div></div>' +
   '<div class="feature-row">' +
   '<div class="feature-box"><img src="' + IMG.tenant + '" alt="Users"/><div><h3>' + u.home.featStaffTitle + '</h3><p>' + u.home.featStaffText + '</p><a href="#/part/05-user-management">' + u.home.featStaffLink + '</a></div></div>' +
   '<div class="feature-box"><img src="' + IMG.admin + '" alt="Admin"/><div><h3>' + u.home.featAdminTitle + '</h3><p>' + u.home.featAdminText + '</p><a href="#/part/11-security-hardening">' + u.home.featAdminLink + '</a></div></div>' +
   '</div>' + cards;
  window.scrollTo(0, 0);
}
...
// Also publish into the Next.js public/ folder so the admin guide is served
// in-app at /admin-guide (see src/app/admin-guide/page.tsx) and opens from
// the sidebar.
const publicGuide = path.resolve(here, "../../../public/admin-guide");
...
console.log(`Built docs/admin-guide/site/index.html — ${PARTS.length} parts × ${SITE_LOCALES.length} languages (${SITE_LOCALES.join(" / ")}), ${WALKS.en.length} walkthroughs per language`);
console.log("Published to public/admin-guide/ (in-app at /admin-guide)");
