...
beforeAll(async () => {
  ...
  const cola = await prisma.posProduct.findUniqueOrThrow({ where: { tenantId_name: { tenantId: "DEFAULT", name: "Coca-Cola can 330ml" } }, include: { stockItem: true } });
  const noodles = await prisma.posProduct.findUniqueOrThrow({ where: { tenantId_name: { tenantId: "DEFAULT", name: "Instant noodles pack" } }, include: { stockItem: true } });
  const print = await prisma.posProduct.findUniqueOrThrow({ where: { tenantId_name: { tenantId: "DEFAULT", name: "Print / scan service" } } });
