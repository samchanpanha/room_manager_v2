 ...
 * via the {@link SessionResolver} SPI (implemented by the kernel module — same
 * lookup as the Next app), and populates the Spring Security context. No
 * session in the cookie (or an invalid one) leaves the context empty, so
 * controllers raise the same 401 the Next handlers do.
 */
@Component
