  ...
  return (
    <div>
      ...
      <PosTerminal
        ...
        openSession={
          openSession
            ? {
                ...
                cashSalesMinor: openSession.sales.filter((s) => s.method === "cash").reduce((sum, s) => sum + s.totalMinor, 0)
              }
            : null
        }
        sales={sales.map((s) => ({
        ...
        categories={[...new Set(products.map((p) => p.category).filter((c): c is string => Boolean(c)))].sort()}
        canWrite={canWrite}
      />
    </div>
  );
