...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const result = await endLease(id, "completed", null);
  ...
  await logAudit({
    ...
    entityId: id,
    summary: `Lease ${lease.code} → completed (${lease.member.party.name}, room ${lease.room.number}): ${result.notes.join("; ")}`,
    propertyId: lease.propertyId,
    before: { status: lease.status },
    after: { status: "completed" },
    ip: clientIp(req)
  });
  return ok({ status: "completed", notes: result.notes });
}
