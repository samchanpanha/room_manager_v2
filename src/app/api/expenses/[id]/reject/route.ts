...
import { rejectExpense } from "@/lib/operations/expenses-service";
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const result = await rejectExpense(id, parsed.data.reason, { id: user.id, name: user.name }, clientIp(req));
